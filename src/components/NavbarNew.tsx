        ...
        <div className={`flex items-center justify-between rounded-2xl border backdrop-blur-xl transition-all duration-300 ${
          ...
          <div className="hidden gap-1 sm:flex">
            {siteConfig.nav.map((link) => (
              ...
              >
                {activeHref === link.href && (
                  <span className="absolute inset-0 rounded-lg border border-cyan-500/50 animate-pulse" />
                )}
                {link.label}
              </a>
            ))}
          ...
          <a
            href="#contact"
            className="rounded-full bg-gradient-to-r from-cyan-500 to-blue-600 px-5 py-2 text-xs font-semibold text-white shadow-lg shadow-cyan-500/50 transition-all hover:shadow-cyan-500/70 hover:scale-105"
          >
            Let&apos;s talk
          </a>
        </div>
